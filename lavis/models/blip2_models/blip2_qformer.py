...
#             print(f'data atts video shape = {data_atts["video"][0].size()}') # [5, 4, 257]
            ...
            # reordered_embeds = torch.cat(embeds[modality])
            # reordered_atts = torch.cat(data_atts[modality])
...
#             print(f'reordered_atts video shape = {reordered_atts.size()}') # [20, 257])
    ...
    def generate(
        ...
        if 'image' in samples:
            ...
            # reordered_embeds = torch.cat(embeds)
            # reordered_atts = torch.cat(data_atts)
            patch_size = reordered_embeds.size()[1]
            reordered_embeds = reordered_embeds.reshape(bs, num, patch_size, -1).contiguous().view(bs, num*patch_size, -1)
            reordered_atts = reordered_atts.reshape(bs, num, patch_size).contiguous().view(bs, num*patch_size)
